...
//str is in byte, while size is in kb
    ...
    int str = pow(2,16);//stride = 4mb
    int ex = 2;
    for (;ex<12;ex++)
{

    int size = pow(2,16);//8kb
    unsigned int array_size = (size/4)*pow(2,10);
    int stride = str/4; // number of offsets in the array, each offset increments 4bytes
    int i = 0;
    //int a[array_size];
...
//	a[i] = 2;
	...
	int j = 0;
	t1 = get_tsc();
	for(j = 0;j<pow(10,6);j++)
	    index = a[index];
        t2 = get_tsc();
        diff = diff_tsc(t1,t2)/pow(10,6);
        printf("%ld\n",diff);
